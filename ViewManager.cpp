...
namespace
{
	...
	// variables for handling speed limits

	const float MIN_CAMERA_SPEED = 0.5f;
	const float MAX_CAMERA_SPEED = 50.0f;
}
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	// adds new speed to movement

	g_pCamera->MovementSpeed += (float)yoffset;

	// handling maximum and minimum speed limits
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// process generate Orthographic/Perspective View
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
	{
		glOrtho(-20.0f, WINDOW_WIDTH, 0.0, WINDOW_HEIGHT, -10.0f, 10.0f);
		glViewport(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT);
		g_pCamera->Position = glm::vec3(0.0f, 1.5f, 9.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
